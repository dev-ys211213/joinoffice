...
function number_format(f, c, h, e) {
    ...
    var b = !isFinite(+f) ? 0 : +f, a = !isFinite(+c) ? 0 : Math.abs(c), k = (typeof e === "undefined") ? "," : e,
        d = (typeof h === "undefined") ? "." : h, j = "", g = function (o, m) {
            var l = Math.pow(10, m);
...
function num_chk(a) {
    ...
    for (i = 0; i < c.length; i++) {
        if ((a.value.charAt(i) >= 0 && a.value.charAt(i) <= 9) || a.value.charAt(i) == "." || a.value.charAt(i) == "-") {
        } else {
            if (LAN == "en") {
...
function search_zip(b) {
    if (document.getElementById("dynamic_js")) {
        var a = document.getElementById("dynamic_js");
        if (a.src == "https://t1.daumcdn.net/mapjsapi/bundle/postcode/prod/postcode.v2.js") {
            search_zip_daum_load(b)
        } else {
            a.src = "https://t1.daumcdn.net/mapjsapi/bundle/postcode/prod/postcode.v2.js";
            a.onload = function () {
                search_zip_daum_load(b)
            }
        }
...
function search_zip_daum_load(eval_put_postno_func_name) {
    if (APP_CONN_YN || MOBILE_CONN_YN) {
        var daum_element_layer_wrap = document.getElementById("search_zip_daum_layer_wrap");
        var daum_element_layer = document.getElementById("search_zip_daum_layer");
        if (!daum_element_layer_wrap) {
            daum_element_layer_wrap = document.createElement("div");
            daum_element_layer_wrap.setAttribute("id", "search_zip_daum_layer_wrap");
            daum_element_layer_wrap.style.position = "fixed";
            daum_element_layer_wrap.style.top = "0px";
            daum_element_layer_wrap.style.left = "0px";
            daum_element_layer_wrap.style.width = "100%";
            daum_element_layer_wrap.style.height = "100%";
            daum_element_layer_wrap.style.zIndex = "1000";
            daum_element_layer_wrap.style.backgroundColor = "rgba(0, 0, 0, 0.7)";
            daum_element_layer_wrap.onclick = function () {
                daum_element_layer_wrap.style.display = "none"
            };
            daum_element_layer = document.createElement("div");
            daum_element_layer.setAttribute("id", "search_zip_daum_layer");
            daum_element_layer.style.position = "absolute";
            daum_element_layer.style.overflow = "hidden";
            daum_element_layer.style.zIndex = "1";
            element_close_btn = document.createElement("img");
            element_close_btn.setAttribute("src", "//t1.daumcdn.net/postcode/resource/images/close.png");
            element_close_btn.style.position = "absolute";
            element_close_btn.style.right = "0px";
            element_close_btn.style.bottom = "0px";
            element_close_btn.style.zIndex = "1";
            element_close_btn.style.cursor = "pointer";
            element_close_btn.onclick = function () {
                daum_element_layer_wrap.style.display = "none"
            };
            daum_element_layer.appendChild(element_close_btn);
            daum_element_layer_wrap.appendChild(daum_element_layer);
            document.body.appendChild(daum_element_layer_wrap)
        }
        setTimeout(function () {
            new daum.Postcode({
                oncomplete: function (data) {
                    ...
                    if (eval_put_postno_func_name) {
                        eval(eval_put_postno_func_name + '("' + data.zonecode + '", "' + fullAddr + '")')
                    } else {
                        new_put_postno(data.zonecode, fullAddr)
                    }
                    daum_element_layer_wrap.style.display = "none"
                }, width: "100%", height: "100%"
            }).embed(daum_element_layer);
            daum_element_layer_wrap.style.display = "block";
            var width = 300;
            var height = 400;
            var borderWidth = 2;
            ...
            daum_element_layer.style.top = "80px"
        }, 500)
    } else {
        new daum.Postcode({
            oncomplete: function (data) {
...
function memo_read(b) {
    ...
    if (APP_CONN_YN == true) {
        app_inweb_url("받은쪽지", "/shop_mypage/s_paper_read.htm?idx=" + b)
    } else {
        if (MOBILE_CONN_YN == true) {
...
function pwd_ch() {
    try {
        if (APP_CONN_YN) {
            app_inweb_url("패스워드 변경", "/shop_mypage/pwd_ch_form.htm")
        } else {
            var b = 440;
            var c = 360;
            ...
            if (MOBILE_CONN_YN == true) {
                ...
                var d = window.open("/shop_mypage/pwd_ch_form.htm", "pwd_ch_win", "top=" + a + ",left=" + g + ",width=" + b + ",height=" + c + ",toolbar=no,scrollbars=no,resizable=no,status=yes,menubar=no,location=no");
                d.focus()
            }
        }
    } catch (f) {
...
function order_address_list() {
    var d = "/sp2/order_move_addr_list.htm";
    if (APP_CONN_YN) {
        app_inweb_url("배송지 관리", d)
    } else {
        var b = 800;
        var e = 700;
        var f = (screen.width - b) / 2;
        var a = (screen.height - e) / 2;
        var c = window.open(d, "order_address_list_win", "top=" + a + ",left=" + f + ",width=" + b + ",height=" + e + ",toolbar=no,scrollbars=yes,resizable=yes,status=yes,menubar=no,location=no");
        c.focus()
    }
}

function review_list_show() {
    var a = "/sp2/review_list.htm";
    if (APP_CONN_YN) {
        app_inweb_url("상품 전체 후기보기", a)
    } else {
        location.href = a
    }
}

function card_set_show() {
    var a = "/sp2/easy_card_set.htm";
    if (APP_CONN_YN) {
        app_inweb_url("간편결제 관리", a)
    } else {
        popup_window(a, 480, 700)
    }
}

function card_list_show() {
    var a = "/sp2/card_list.htm";
    popup_window(a, 480, 700)
}

function card_reg() {
    var a = "/sp2/card_form.htm?card_reg_yn=1";
    popup_window(a, 480, 700)
}
...
function pwd_search() {
    ...
    try {
        if (APP_CONN_YN) {
            if (ios_yn) {
                if (app_version_code >= 120) {
                    f = 1
                }
            } else {
                if (app_version_code >= 158) {
                    f = 1
                }
            }
        }
        if (f) {
            app_outweb_url("ID/패스워드 찾기", "/shop_popup/pwd_search_form.htm?app_win_name=" + APP_window_name)
        } else {
            if (MOBILE_CONN_YN == true) {
                ...
                var a = (screen.height - d) / 2;
                var b = window.open("/shop_popup/pwd_search_form.htm", "pwd_search_win", "top=" + a + ",left=" + j + ",width=" + c + ",height=" + d + ",toolbar=no,scrollbars=no,resizable=no,status=yes,menubar=no,location=no");
                b.focus()
            }
        }
    } catch (g) {
...
function popup_window_close() {
    try {
        if (APP_CONN_YN && app_version_code >= 158) {
            app_submenu_hide()
        } else {
            try {
...
function admin_seller_pop() {
    try {
        if (APP_CONN_YN) {
            ...
            var g = 0;
            var a = 0;
            var c = window.open("/admin_om/", "", "top=" + a + ",left=" + g + ",width=" + b + ",height=" + d + ",toolbar=no,scrollbars=yes,resizable=yes,fullscreen=no,status=yes,menubar=no,location=no");
            c.focus()
        }
    } catch (f) {
        var b = 1100;
...
function shop_view(b, d, c) {
    ...
    if (!b) {
        ...
        window.open("/shop_admin/shop_link.php?mobile_conn_chk=off&shop_id=" + b + "&magic_all_idx=" + d + "&sel_lan=" + c)
    }
}
...
function naver_map(f, a, b, e, d, c) {
    if (!a) {
        a = 300
    }
    if (!b) {
    ...
    if (location.host == "www.local.co.kr") {
        document.write("<iframe id='mapContainer' src='http://www.local.co.kr/sys_load/naver_map_xy_load.php?map_w=" + a + "&map_h=" + b + "&addr=" + f + "&title=" + e + "&info=" + d + "&zoom=" + c + "' width='" + a + "' height='" + b + "' frameborder='0' scrolling='no' marginheight='0' marginwidth='0' ></iframe>")
    } else {
        document.write("<iframe id='mapContainer' src='//map.anybuild.com/sys_load/kakao_map_xy_load.php?map_h=" + b + "&addr=" + f + "&title=" + e + "&info=" + d + "&zoom=" + c + "' width='" + a + "' height='" + b + "' frameborder='0' scrolling='no' marginheight='0' marginwidth='0' ></iframe>")
    }
}
...
function naver_map_output(c) {
    ...
    var e = {width: naver_map_w, height: naver_map_h, mapMode: 0};
        ...
        if (APP_CONN_YN) {
            ...
            if (c) {
                setInterval(function () {
                    app_toast(1, "파일업로드 중입니다. 잠시만 기다려주세요.")
                }, 3000);
                app_toast(1, "파일업로드 중입니다. 잠시만 기다려주세요.")
            }
            if (ios_yn) {
...
function auto_iframe_load(c, d, b) {
    if (!d) {
        d = 0
    }
    if (!b) {
        ...
        if (APP_CONN_YN) {
            if (ios_yn) {
                if (d == 1) {
                    ...
                    setTimeout(function () {
                        var e = create_iframe();
                        document.getElementById(e).src = c
                    }, 200)
                } else {
                    var a = create_iframe();
...
function ae_file_down(a, c) {
    if (APP_CONN_YN == true) {
        app_download("/html_editor/aram_editor/file_download.php?idx=" + a, c, 1)
    } else {
        if (MOBILE_CONN_YN == true) {
            location.href = "/html_editor/aram_editor/file_download.php?idx=" + a
        } else {
            var b = create_iframe();
            document.getElementById(b).src = "/html_editor/aram_editor/file_download.php?idx=" + a
        }
    }
}

function content_link_chk(c, k, g) {
    if (c.indexOf("http") == 0) {
        var b = location + "";
        var a = b.split("/");
        var d = a[0] + "/" + a[1] + "/" + a[2];
...
function web_app_photo_view(c, b) {
    var h = (screen.width - 640) / 2;
    var a = (screen.height - 480) / 2;
    var d = 0;
    if (!b) {
        b = 0
    }
    ...
    if (d == 1) {
        app_outweb_url("상세보기", "/APP/photo_slide_view.htm?download_yn=" + b + "&photo_url_list_b64=" + c)
    } else {
        var f = window.open("/APP/photo_slide_view.htm?download_yn=" + b + "&photo_url_list_b64=" + c, "web_app_photo_view_win", "resizable=yes,toolbar=no,width=430,height=500,scrollbars=yes,top=" + a + ",left=" + h + "");
        f.focus()
    }
}

function company_addr_img_sms() {
    if (document.getElementsByName("ca_sms_hp1").length) {
        var e = document.getElementsByName("ca_sms_hp1")[0].value;
        var d = document.getElementsByName("ca_sms_hp2")[0].value;
        var c = document.getElementsByName("ca_sms_hp3")[0].value;
        if (!e || !d || !c) {
    ...
    if (document.getElementsByName("ca_sms_hp").length) {
        var f = document.getElementsByName("ca_sms_hp")[0].value;
        if (!f) {
            alert("휴대전화 번호를 입력해 주세요");
            return
        }
    ...
    if (confirm("약도를 메세지로 받으시겠습니까?")) {
        var a = create_iframe();
        document.getElementById(a).src = "/shop_info/company_addr_sms_exec.php?hp_num=" + b
    }
}

function img_masking_pop(e) {
    var g = "/etc/img_masking_form.htm?column_name=" + e;
    if (document.getElementsByName(e).length > 0) {
        g += "&sel_img_file_name=" + document.getElementsByName(e)[0].value.trim()
    }
    var b = 600;
    var c = 815;
    var f = (screen.width - b) / 2;
    var a = (screen.height - c) / 2;
    var d = window.open(g, "img_masking_pop_win", "top=" + a + ",left=" + f + ",width=" + b + ",height=" + c + ",toolbar=no,scrollbars=yes,resizable=yes,status=yes,menubar=no,location=no");
    return d
}

function img_masking_receive(b, a) {
    if (!a) {
        return false
    }
    a = a.trim();
    if (document.getElementsByName(b).length > 0) {
        document.getElementsByName(b)[0].value = a
    }
};